...
from KARMAN_classism import *
from KARMAN_contants import *

# Constants - sets the size of the window
window_bounds = WIDTH, HEIGHT, scale = 600, 600, .002
origin = x0, y0 = WIDTH/2, HEIGHT-HEIGHT/2  # This is the new origin
temp = 0


def ball_xy(ball, num, maxnum):
    return 10 + num * ((WIDTH-20)/maxnum), origin[1] - ball.pos.y*scale
...
def drawer(object_list, place_to_draw_stuff=screen):
    for i in range(len(object_list)):
        p.draw.circle(place_to_draw_stuff, object_list[i].color.color(), ball_xy(object_list[i], i, len(object_list)), 1)


populations = [Population(10, 2000, Ball(90, Vec(0, 8848.86), speed=330, wind=Vec(0, 0, 0)))]
...
while True:
    ...
    if running:
        ...
        p.draw.line(screen, (255, 255, 255), (0, y0 - (100000 * scale)), (WIDTH, (y0 - 100000 * scale)))
        p.display.flip()
