...
from A_APPROACH_contants import *
from vectors import *
...
class Ball:
    ...
    def variedcopy(self, randomness):
        ...
        new_speed = random.gauss(self.speedinit, randomness)
    ...
    def __repr__(self):
        return f"Ball with {self.wind} that landed with an x vel of {self.vx} landed {self.range}m away from the target. Angle equaled: {self.angle} Speed: {self.speedinit}m/s"


class Population:
    def __init__(self, popsize, randomness, defaultball: Ball, survival_rate = 10):
        self.population = [Ball(random.randrange(0, 90), defaultball.pos, mag(defaultball.v) + random.randrange(-10, 10), wind=defaultball.wind, spin=defaultball.spin) for i in range(popsize)]
    ...
    def reproduction(self):
        ...
        self.population = sorted(self.population, key=lambda x: x.speed)
